                ...
                @if( ___getAccessButton([]) )
                    ...
                    <a href="{{ route('event-create') }}">
                        <button class="btn btn-success elevation-2">
                            <i class="bx bx-fw bxs-plus-circle"></i> Nuevo evento
                        </button>
                    </a>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            @foreach( $rows as $key => $row )
                                <tr>
                                    <td>
                                        <a href="{{ route('event-update', [ 'id' => $row->id ]) }}" title="Editar elemento">  {{ $row->name }} </a>
                                    </td>
                                        ...
                                        @if ( !is_null( $row->schools ) )
                                            ...
                                            @foreach ( ___getNameSchools( json_decode($row->schools)) as $school )
                                                ...
                                                @if( $row->status == 1 && $school->server_name )
                                                    <a href="{{ $school->server_name . '/eventos/' . $row->slug }}" title="Ir al sitio" target="_blank">
                                                        <h6>
                                    ...
                                    <td wire:key="{{ $row->id }}" class="text-right" wire:ignore>
                                        ...
                                        @if( ___getAccessButton( json_decode($row->schools) ) )
                                            ...
                                            <a class="btn btn-circle btn-primary elevation-2 mr-2" title="Editar" href="{{ route('event-update', [ 'id' => $row->id ]) }}">
