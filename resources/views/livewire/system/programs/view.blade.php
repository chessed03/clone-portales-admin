                ...
                @if( ___getAccessButton([]) )
                    ...
                    <a href="{{ route('program-create') }}">
                        <button class="btn btn-success elevation-2">
                            <i class="bx bx-fw bxs-plus-circle"></i> Nuevo programa
                        </button>
                    </a>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            @foreach( $rows as $key => $row )
                                <tr>
                                    <td>
                                        <a href="{{ route('program-update', [ 'id' => $row->id ]) }}" title="Editar elemento">  {{ $row->name }} </a>
                                    </td>
                                    <td class="text-center">

                                        @foreach ( ___getNameSchools( [$row->school_id]) as $school )

                                            @if( $row->status == 1 && $school->server_name )
                                                <a href="{{ $school->server_name . '/programas/' . $row->slug }}" title="Ir al sitio" target="_blank">
                                                    <h6>
                                    ...
                                    <td wire:key="{{ $row->id }}" class="text-right" wire:ignore>
                                        ...
                                        @if( ___getAccessButton( [ $row->school_id ] ) )
                                            ...
                                            <a class="btn btn-circle btn-primary elevation-2 mr-2" title="Editar" href="{{ route('program-update', [ 'id' => $row->id ]) }}">
