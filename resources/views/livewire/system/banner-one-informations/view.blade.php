    ...
    <div class="card-header">
        ...
        <div class="row">
            ...
            <div class="col-6">
                ...
                <h6>Lista de Cintas informativas 1</h6>
            ...
            <div class="col-6 text-right">
                ...
                @if( ___getAccessButton( [] ) )
                    ...
                    <a href="{{ route('bannerOneInformation-create') }}">
                        <button class="btn btn-success elevation-2">
                            <i class="bx bx-fw bxs-plus-circle"></i> Nueva información
                        </button>
                    </a>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            @foreach( $rows as $key => $row )
                                <tr>
                                    <td>
                                        <a href="{{ route('bannerOneInformation-update', [ 'id' => $row->id ]) }}" title="Editar elemento"> {{ $row->title }} </a>
                                    </td>
                                    <td class="text-center" wire:key="check-{{ $row->id }}">

                                        <div class="custom-control custom-switch custom-switch-on-success ml-4">
                                            <input
                                                type="checkbox"
                                                class="custom-control-input check-banner-{{ $row->id }}"
                                                wire:click="setActiveStatus({{ $row->id  }})"
                                                id="checkBoxStatus{{ $row->id }}"
                                                value="{{ $row->id }}" {{ ( $row->status == 1 ) ? 'checked' : '' }} @if( !___getAccessButton( [ $row->school_id ] ) )  disabled="" @endif
                                            >
                                    ...
                                    <td class="text-right" wire:key="dropdown-{{ $row->id }}">

                                        @if( ___getAccessButton( [ $row->school_id ] ) )

                                            <a class="btn btn-circle btn-primary elevation-2 mr-2" title="Editar" href="{{ route('bannerOneInformation-update', [ 'id' => $row->id ]) }}">
