    ...
    <div class="card-header">
        ...
        <div class="row">
            ...
            <div class="col-6">
                ...
                <h6>Lista de carrusel de imágenes</h6>
            ...
            <div class="col-6 text-right">
                ...
                @if( ___getAccessButton( [] ) )
                    ...
                    <a href="{{ route('carousel-image-create') }}">
                        <button class="btn btn-success elevation-2">
                            <i class="bx bx-fw bxs-plus-circle"></i> Nueva imágen
                        </button>
                    </a>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        @foreach( $rows as $key => $row )
                            <tr>
                                <td >
                                    <a href="{{ route('carousel-image-update', [ 'id' => $row->id ]) }}" title="Editar elemento">  {{ $row->name }} </a>
                                </td>
                                <td class="text-center">

                                    @if ( !is_null( $row->schools ) )

                                        @foreach ( ___getNameSchools( $row->schools ) as $school )

                                            @if( $row->status == 1 && $school->server_name )
                                                <a href="{{ $school->server_name }}" title="Ir al sitio" target="_blank">
                                                    <h6>
                                ...
                                <td wire:key="{{ $row->id }}" class="text-right" wire:ignore>
                                    ...
                                    @if( ___getAccessButton( $row->schools ) )

                                        <a class="btn btn-circle btn-primary elevation-2 mr-2" title="Editar" href="{{ route('carousel-image-update', [ 'id' => $row->id ]) }}">
